...
;                  ,(pock `(,subj ,(cadr args))))))
;          ((equal op (read-noun "?")) subj))))
(defun pock1 (noun) (pock (car noun) (cadr noun)))

(defun check-pock (src ref) 
  (let ((res (pock1 (read-noun src))) 
...
;(format-noun nil (pock (read-noun (load-src "src.pock"))))
;(format-noun nil (pock (read-noun "[[T T T] [$]]")))
;(format-noun nil (pock (read-noun "[[T T T] [< [$]]]")))
;(format-noun nil (pock (read-noun "[[T T T] [> [$]]]")))
;(format-noun nil (pock (read-noun "[[T [T T] T] [< [> [$]]]]")))
;(format-noun nil (pock (read-noun "[[T T T] [' [T ~ T]]]")))
;(format-noun nil (pock (read-noun "[[T T T] [: [' [T ~ T]] [> [$]]]]")))
;(format-noun nil (pock (read-noun "[[T T T] [* [' [T ~ T]] [ '[> [$]]]]]")))
;(format-noun nil (pock (read-noun "[[T T T] [? [< [$]] [' [T ~ T]] [> [$]]]]")))
